...
export default function NyttBlogginnleggPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    console.log("[v0] Submitting blog post:", formData)
    setError(null)

    const { error: insertError } = await supabase.from("blog_posts").insert([formData])

    console.log("[v0] Insert result:", { error: insertError })

    if (insertError) {
      console.error("[v0] Insert error:", insertError)
      setError("Feil ved lagring: " + insertError.message)
    } else {
  ...
  return (
    <div className="p-8">
      <h1 className="text-3xl font-light mb-8">Nytt blogginnlegg</h1>

      {error && (
        <div className="max-w-2xl mb-6 p-4 bg-red-50 border border-red-200 rounded-lg text-red-800">{error}</div>
      )}
